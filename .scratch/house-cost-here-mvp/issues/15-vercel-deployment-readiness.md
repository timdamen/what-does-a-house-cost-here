...
- Deployed 2026-09-18 with `vercel deploy --prod --yes --scope focusring` from the repo root. Production alias: https://what-does-a-house-cost-here.vercel.app. Verified live: `/` and a deep link return 200; `/api/houses`, `/api/facts` (GB with Land Registry summary, NL with `priceSummary: null`) and `/api/geocode` return real open data with `cache-control: public, max-age=21600` and Vercel edge cache hits.
