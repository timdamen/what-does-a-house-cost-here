...
- Loading: a dynamic `import('maplibre-gl')` (and its CSS) inside a `.client.vue` component wrapped in `<ClientOnly>`, run only after Location is known. Not Nuxt Scripts: `useScript` injects a script URL and suits third-party tags, while maplibre-gl 6 ships ESM only (no UMD `dist/maplibre-gl.js`). Vite emits the import as its own chunk, so nothing lands in the entry bundle.
- Placeholder: the server-rendered placeholder (`MapPlaceholder.vue`) is a sibling of `<ClientOnly>` inside `HouseMapFrame.vue`, not its `#fallback` slot. A `#fallback` node is replaced on hydration, and Chrome then drops it as a largest-contentful-paint candidate and attributes the LCP to whatever paints after the map chunk. As a sibling the node the server rendered is hydrated in place, so the paragraph painted with the HTML stays the page's LCP (ticket 13). The map component reports its loading and error state upward; the frame keeps the placeholder until the map is ready.
...
- The map chunk, and the area requests, start only after the shell's first paint (`useAfterFirstPaint`: two frames after hydration). The largest text is server-rendered and painted from the HTML; Lighthouse's simulated throttling charges every request started before that paint against the largest contentful paint, and with the map chunk in that path the deep link read 2.8 s instead of 2.4 s depending on a few milliseconds of timing. A frame or two of delay is invisible to the visitor and keeps the budget deterministic.
